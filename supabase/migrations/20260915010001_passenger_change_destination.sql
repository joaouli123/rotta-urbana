-- Rotta Urbana — allow the PASSENGER (not just the driver) to change the
-- destination of their own active ride. Same recalculation rule as before
-- (distance/duration/price via fare_estimate), only the authorization check
-- changes: either the assigned driver or the assigned passenger may call it.
...
comment on function public.update_ride_destination is
  'Allows the assigned driver OR the assigned passenger to change an active ride destination inside the configured service area and recalculates its fare.';
