-- ============================================================================
-- Rotta Urbana — push "Seu motorista chegou" to the passenger
-- ----------------------------------------------------------------------------
-- Only the accept (searching → driver_on_way) was pushed from the server. When
-- the driver taps "Cheguei ao passageiro" a passenger with the app closed got
-- nothing. Same trigger, one more transition: driver_on_way → driver_arrived.
-- ============================================================================
...
begin
  if new.status = 'driver_on_way' and coalesce(old.status, '') = 'searching' then
  ...
  elsif new.status = 'driver_arrived' and coalesce(old.status, '') <> 'driver_arrived' then
    select push_token into v_token
    from public.profiles
    where id = new.passenger_id and push_token is not null;
    ...
    if v_token is not null then
      ...
      v_body := 'Ele está no local de embarque. Entre no veículo'
        || case when v_veh is not null then ' • ' || v_veh else '' end
        || case when v_plate is not null then ' • ' || v_plate else '' end;
      ...
      perform net.http_post(
        ...
        body    := jsonb_build_object(
          ...
          'priority', 'high',
          'channelId', 'ride-status',
          'data', jsonb_build_object('rideId', new.id, 'type', 'driver_arrived')
        )
      );
    end if;
  end if;
...
comment on function public.notify_ride_accepted is
  'AFTER UPDATE on rides: Expo push to the passenger when a driver accepts (searching → driver_on_way) and when the driver arrives (→ driver_arrived).';
