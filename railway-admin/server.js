...
adminRouter.get('/subscriptions', requireAuth, async (req, res) => {
  ...
  const filters = `<div class="card subscription-filters"><div class="subscription-status-filters"><a class="${statusFilter === 'all' ? 'on' : ''}" href="${href('all')}">Todas (${allSubs.length})</a><a class="${statusFilter === 'active' ? 'on' : ''}" href="${href('active')}">Ativas (${count('active')})</a><a class="${statusFilter === 'pending' ? 'on' : ''}" href="${href('pending')}">Pendentes (${count('pending')})</a><a class="${statusFilter === 'expired' ? 'on' : ''}" href="${href('expired')}">Vencidas (${count('expired')})</a><a class="${statusFilter === 'suspended' ? 'on' : ''}" href="${href('suspended')}">Suspensas (${count('suspended')})</a></div><form class="subscription-search" method="get" action="/subscriptions"><input name="q" value="${esc(req.query.q || '')}" placeholder="Buscar motorista ou ID"><input type="hidden" name="status" value="${esc(statusFilter)}"><button class="act gray" type="submit">Pesquisar</button></form></div>`;
